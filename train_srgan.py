...
import model
import srgan_config
from dataset import CPUPrefetcher, TrainValidImageDataset, TestImageDataset, TrainCustomDataset
...
def main():
    ...
    psnr_model = psnr_model.to(device='cpu')
    ssim_model = ssim_model.to(device='cpu')
    sam_model = sam_model.to(device='cpu')
...
def build_model():
    ...
    d_model = d_model.to(device='cpu')
    g_model = g_model.to(device='cpu')
...
def define_loss():
    ...
    # Transfer to CUDA
    pixel_criterion = pixel_criterion.to(device='cpu')
    content_criterion = content_criterion.to(device='cpu')
    adversarial_criterion = adversarial_criterion.to(device='cpu')
...
def train(
    ...
    while batch_data is not None:
        ...
        gt = batch_data["gt"].to(device='cpu', non_blocking=True)
        lr = batch_data["lr"].to(device='cpu', non_blocking=True)

        # Set the real sample label to 1, and the false sample label to 0
        batch_size, _, height, width = gt.shape
        real_label = torch.full([batch_size, 1], 1.0, dtype=gt.dtype, device='cpu')
        fake_label = torch.full([batch_size, 1], 0.0, dtype=gt.dtype, device='cpu')
