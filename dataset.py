...
class TrainCustomDataset(Dataset):
    ...
    Args:
        gt_image_dir (str): Train highres dataset address.
        lr_image_dir (str): Train lowres dataset address.
        gt_image_size (int): Highres resolution image size.
        lr_image_size (int): Lowres resolution image size.
    ...
    def __init__(
        ...
        super(TrainCustomDataset, self).__init__()
        self.high_image_file_names = [os.path.join(gt_image_dir, image_file_name) for image_file_name in
                                 os.listdir(gt_image_dir)]
        self.low_image_file_names = [os.path.join(lr_image_dir, image_file_name) for image_file_name in
                                 os.listdir(lr_image_dir)]
    ...
    def __getitem__(self, batch_index: int):
        ...
        gt_image = cv2.imread(self.high_image_file_names[batch_index]).astype(np.float32) / 255.
        lr_image = cv2.imread(self.low_image_file_names[batch_index]).astype(np.float32) / 255.

        if self.gt_image_size != 96 or self.lr_image_size != 24:
            raise ValueError("Unsupported train data size")

        # BGR convert RGB
        gt_color_image = cv2.cvtColor(gt_image, cv2.COLOR_BGR2RGB)
        lr_color_image = cv2.cvtColor(lr_image, cv2.COLOR_BGR2RGB)

        # Convert image data into Tensor stream format (PyTorch).
        # Note: The range of input and output is between [0, 1]
        gt_tensor = imgproc.image_to_tensor(gt_color_image, False, False)
        lr_tensor = imgproc.image_to_tensor(lr_color_image, False, False)

        return {"gt": gt_tensor, "lr": lr_tensor}

    def __len__(self) -> int:
        if len(self.high_image_file_names) != len(self.low_image_file_names):
            raise ValueError("Data incosistency")
        return len(self.high_image_file_names)
...
class CPUPrefetcher:
    """Use the CPU to accelerate data reading.
    ...
    def preload(self):
        ...
        for k, v in self.batch_data.items():
            if torch.is_tensor(v):
                # Move tensors to CPU
                self.batch_data[k] = self.batch_data[k].cpu()
